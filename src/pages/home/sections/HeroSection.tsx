...
import type { HeroContent, HeroStat } from "@/shared/types/hero";
...
export default function HeroSection() {
    ...
    const summary = hero.summary[language];
            ...
            <div className="grid w-full grid-cols-1 items-center gap-8 lg:grid-cols-[minmax(0,1fr)_26rem] lg:gap-12">
                <div className="max-w-116 space-y-5">
                    <div className="flex flex-wrap gap-2">
                        <Badge icon={<MapPin size={12} />} label={hero.location[language]} />
                        <Badge icon={<Terminal size={12} />} label={hero.roleBadge[language]} />
                    </div>
                    ...
                    >
                        ...
                        <h1 className="text-9xl font-bold tracking-tighter leading-[0.86] md:text-7xl xl:text-[6.5rem]">
                            {hero.headline.firstName}
                            <br />
                            <span>{hero.headline.lastName}</span>
                        </h1>
                        <motion.p
                        ...
                        >
                            <>
                                {summary.beforeHighlightOne}
                                <strong className="text-black font-semibold">{summary.highlightOne}</strong>
                                {summary.betweenHighlights}
                                <strong className="text-black font-semibold">{summary.highlightTwo}</strong>
                            </>
                        </motion.p>
                    </motion.div>
                ...
                >
                    <div className="group relative h-80 w-72 rounded-3xl md:h-104 md:w-88 xl:h-112 xl:w-96">
                        ...
                        <div className="absolute -bottom-8 -left-10 z-10 hidden gap-4 md:flex">
                            {hero.stats.map((stat: HeroStat) => (
                                <StatCard
                                    key={stat.id}
                                    icon={stat.id === "experience" ? <Code2 size={16} /> : <FolderGit2 size={16} />}
                                    value={stat.value}
